  ...
  return (
    <div className="p-4 sm:p-8 max-w-7xl mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6 mb-8 sm:mb-12">
        <div>
          <h1 className="text-3xl sm:text-4xl font-black mb-2">{t('myTournaments')}</h1>
          <p className="text-zinc-500 font-medium">{t('manageCompetitions')}</p>
        </div>
        {onCreateNew && (
          <button
            onClick={onCreateNew}
            className="flex items-center justify-center gap-2 px-6 py-3 bg-green-500 text-black font-black rounded-xl hover:bg-green-400 transition-all shadow-[0_0_20px_rgba(34,197,94,0.3)] w-full sm:w-auto"
          >
      ...
      {tournaments.length === 0 ? (
        <div className="text-center py-20 sm:py-32 bg-zinc-900/30 border border-dashed border-zinc-800 rounded-[32px] sm:rounded-[40px]">
          ...
          {onCreateNew && (
            <button
              onClick={onCreateNew}
              className="px-8 py-3 bg-zinc-800 text-white font-bold rounded-xl hover:bg-zinc-700 transition-all w-full sm:w-auto"
            >
              {t('getStarted')}
            </button>
          )}
