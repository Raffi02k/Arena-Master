...
import { Trophy, Users, Settings2, Play, Hash, Target, GitBranch, Shuffle, Rows3 } from 'lucide-react';
import {
  TournamentFormat,
  Team,
  SwissInitialPairing,
...
export const TournamentSetup: React.FC<TournamentSetupProps> = ({ t, onStart }) => {
  ...
  const hasEvenGroupCount = groupCount >= 2 && groupCount % 2 === 0;
  ...
  const manualGroupSizes = groupNames.reduce<Record<string, number>>((acc, groupName) => {
    acc[groupName] = teams.filter((team) => manualGroupAssignments[team.id] === groupName).length;
    return acc;
  }, {});
  ...
  const isValid =
    ...
    teams.length >= 2 &&
    (!isKnockout || isEven) &&
    (!isGroupKnockout || (isGroupSizeValid && hasEvenGroupCount && hasValidManualGroups));
  ...
  const handleStart = () => {
    ...
    const teamsWithGroups = teams.map((team) => ({
      ...team,
    ...
    <div className="max-w-4xl mx-auto p-4 sm:p-6 space-y-8 sm:space-y-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-8">
          ...
          <AnimatePresence>
            {isGroupKnockout && (
              ...
              >
                ...
                {isGroupSizeValid && !hasEvenGroupCount && (
                  <div className="rounded-2xl border border-red-500/30 bg-red-500/10 p-4 text-sm text-red-300">
                    You need an even number of groups so qualifiers can pair Group A vs B, C vs D, and so on.
                  </div>
                )}
        ...
        <div className="space-y-6 bg-zinc-900 p-6 sm:p-8 rounded-3xl border border-zinc-800 shadow-2xl relative overflow-hidden group">
          ...
          <div className="max-h-[380px] sm:max-h-[420px] overflow-y-auto space-y-2 pr-2 custom-scrollbar">
                ...
                >
                  ...
                  {isGroupKnockout && groupAssignmentMethod === GroupAssignmentMethod.MANUAL && groupNames.length > 0 && (
                    <select
                      ...
                      className="bg-zinc-900 border border-zinc-800 rounded-xl px-3 py-2 text-xs font-bold text-zinc-100"
                    >
